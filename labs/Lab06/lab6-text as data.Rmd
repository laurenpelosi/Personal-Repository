...
path_in <- "/Users/laurenpelosi/Desktop/git/Personal-Repository/labs/Lab06/DickinsonPoems.csv"
poems <- read_csv("DickinsonPoems.csv") %>%
  filter(text != "Missing")
```
...
> ANSWER: Because the stop words are appearing in all documents, the idf will be low. The tf-idf will also be low. This low tf-idf is telling us that the word isn't very significant to the document, which is true of stop words. 
...
#the seed pulls a reproducible sample (just for this purpose)
...
> ANSWER: The range of each word's meaning changes over time, so while Emily could have intended one connotation for a word, that meaning could have changed by now. 

- The lexicons are based on unigrams.  Do you see any disadvantages of basing the sentiment on single words?

> ANSWER: The words around a single word can negate or alter its meaning, so bigrams or n-grams are better-suited for sentiment analysis. 
...
###Anti-join will keep all the unique words
```
