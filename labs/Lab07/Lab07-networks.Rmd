...
```{r}
path_in <- "/Users/laurenpelosi/Desktop/git/Personal-Repository/labs/Lab07"
...
> ANSWER: It would be an edge attribute. 
...
```{r}
#Specifying which package degree is coming from (igraph)
igraph::degree(mig_easteurp)

#How many countries are people migrating to from Hungary?
igraph::degree(mig_easteurp, mode = "out")

#People are coming from 8 different countries into Hungary. 
...
```{r}
#total movement
#sort() puts it in increasing order
sort(strength(mig_easteurp, weights = E(mig_easteurp)$Y2000))

# in
#Number of women entering a country in Year 2000. 
sort(strength(mig_easteurp, weights = E(mig_easteurp)$Y2000
         , mode = "in"))
...
> ANSWER: There are 708 vertices (nodes), or 708 characters. 

```{r}
# create as network object
got_graph <- graph_from_data_frame(got
                                   , directed = FALSE)
...
> ANSWER:  There are 1411 edges, or character interactions. 
...
```{r}
diameter(got_graph, directed = FALSE)
```
