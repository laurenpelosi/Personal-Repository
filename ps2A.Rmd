...
flightsJoined2 <- flights %>%
  mutate(distance/1.6 == "distance_km") %>%
  filter(name, flight, arr_delay, distance_km) %>%
  filter(select(distance_km < 500))
...
> ANSWER: Unable to get babynames2 to save because of ungrouping output error. If this ran, I would take the top three names. 
