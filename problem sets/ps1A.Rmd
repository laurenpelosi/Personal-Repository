...
g <- ggplot(CIACountries, aes(y=gdp, x=educ)) + geom_text(aes(label=country, color = net_users), size = 3)
...
```{r}
g+xlab("% of GDP Spent on Education")+
...
> ANSWER: reorder(drg, mean_charge) arranges the drugs in order along the x axis according to the mean charge, or their y-value. When you replace x = reorder(drg, mean_charge) with x=drg, the drugs are no longer arranged in order of their mean charge, and the aesthetic changes. 
...
> ANSWER:  The theme line rotated the x-axis labels 90 degrees so that they were readable; without the line, the labels overlap. 
...
> ANSWER: 
1.) I'm having trouble conceptually understanding the use of ifelse. Without the code, I would have thought we needed "if". Where does the "else" come in? 
2.) In the creation of the many_names_plot, what does the %in% operator do? (Also, how does the %+% operator differ from %>%?)
